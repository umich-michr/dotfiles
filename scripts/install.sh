...
while [[ "$#" -gt 0 ]]; do
  ...
  *)
    err "Unknown parameter passed: $1"
    exit 1
    ;;
  esac
  shift
done
export DRY_RUN FORCE # Export them so link.sh inherits them automatically
...
install_brew() {
  if [[ "$PLATFORM" == "macos" ]]; then
    # If the file exists, just set up the environment
    if [[ -x /opt/homebrew/bin/brew ]]; then
      eval "$(/opt/homebrew/bin/brew shellenv)"
    # Otherwise, if brew isn't in the PATH, install it
...
install_zsh() {
  if command -v zsh >/dev/null 2>&1; then
    if zsh -ic 'echo $ZSH_VERSION' >/dev/null 2>&1; then
      ok "zsh installed and launches correctly"
      return 0
    else
      warn "zsh present but fails to launch correctly — attempting install/reinstall"
    fi
  fi
...
install_zim() {
  ...
  ZIM_HOME="$ZIM_HOME" zsh "$ZIM_HOME/zimfw.zsh" init -q
}
...
install_mise() { run bash -c "curl https://mise.run | sh"; }

install_starship() { run bash -c "curl -sS https://starship.rs/install.sh | sh -s -- -y"; }
